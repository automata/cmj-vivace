...
ChucK~\cite{wang2003chuck} and a dedicated mixing Puredata + Analog
...
coding has been gaining world wide popularity~\cite{nilson2007live,
...
manifesto~\cite{freak}, other written resources, and in spoken
instances.
...
Puredata. Fluxus~\cite{fluxus} was also inspirational for the Vivace
...
After the creation of Audio Data API~\cite{audiodata} and the most
recent Web Audio API~\cite{webaudio} -- enabling real time audio
...
livecodelab~\cite{livecodelab}, to note some of them.
...
Vivace editor collaborative we used ShareJS~\cite{sharejs} which make
